...
int flv_read_metadata(AVFormatContext *s);
...
/*
        case FLV_CODECID_NELLYMOSER:
            ...
            */
        default:
            printf("Unsupported audio codec (%x)\n", flv_codecid >> FLV_AUDIO_CODECID_OFFSET);
...
static int flv_set_video_codec(AVFormatContext *s, AVStream *vstream, int flv_codecid) {
    ...
    switch(flv_codecid) {
		...
		*/
        ...
        default:
            printf( "Unsupported video codec (%x)\n", flv_codecid);
...
int amf_parse_object(AVFormatContext *s, AVStream *astream, AVStream *vstream, const char *key, int64_t max_pos, int depth) {
    ...
    switch(amf_type) {
        case AMF_DATA_TYPE_NUMBER:
			val =get_be64(ioc);
            num_val = av_int2dbl(val);
//			num_val_g = (int)num_val;
		//	printf("val %x,val= %lld,num_val %f\n ",val,val,num_val);
		//	printf("%s %d num_val_g %d,num_val %fn",__FUNCTION__,__LINE__,num_val_g,num_val);
			break;
...
//printf("val %f,val=lld %d,num_val %f\n ",val,val,num_val);
...
//printf("num_val %f\n",num_val);
            ...
            //else if(!strcmp(key, "duration")) {s->duration = num_val * 1000;
            else if(!strcmp(key, "filesize")) s->file_size= num_val;
            else if(!strcmp(key, "width")  && s ) vstream->width  = num_val;
            else if(!strcmp(key, "height") && s ) vstream->height = num_val;
            ...
            else if(!strcmp(key, "framerate") && vstream && num_val > 0){
				vstream->r_frame_rate.num= num_val;
				vstream->r_frame_rate.den= 1;
			//	printf("val %f,val=lld %d,num_val %f\n ",val,val,num_val);
            }
...
static AVStream *create_stream(AVFormatContext *s, int is_audio){
	...
	if(st->codec== NULL){
		st->codec =av_mallocz_t(sizeof(AVCodecContext));
		if(st->codec== NULL){
			av_free_t(st);
			return NULL;
		}
	...
	st->codec_type = st->codec->codec_type;
	st->index = s->nb_streams -1;
    av_set_pts_info(st, 32, 1, 1000); /* 32 bit pts in ms */
	//s->streams[s->nb_vstreams] = st;

    return st;
}

int flv_read_header(AVFormatContext *s,
                           AVFormatParameters *ap)
{
    ...
    if (!flags) {
        ...
        printf( "Broken FLV file, which says no streams present, this might fail\n");
    }
...
int flv_read_header_allInfo(AVFormatContext *s)
{
    int offset, flags;
...
int flv_read_release(AVFormatContext *s)
{
	if(s)
	{	
		int i;
		...
		if(s->streams[i])
		{
			...
			{
				av_free_t(s->streams[i]->codec);
				s->streams[i]->codec = NULL;
			
			}
			av_free_t(s->streams[i]);
			s->streams[i] = NULL;
		}
	}
	return 0;
...
{
    av_free_t(st->codec->extradata);
    st->codec->extradata = av_mallocz_t(size + FF_INPUT_BUFFER_PADDING_SIZE);
...
int flv_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (st->codec->codec_id == CODEC_ID_AAC ||
        ...
        if (st->codec->codec_id == CODEC_ID_H264) {
            ...
            if (cts < 0) { // dts are wrong
                flv->wrong_dts = 1;
                printf( "negative cts, previous timestamps might be wrong\n");
            }
            if (flv->wrong_dts)
                dts = AV_NOPTS_VALUE;
        }
...
int flv_read_seek(AVFormatContext *s, int stream_index,int64_t ts, int flags)
{
...
int flv_seek_frame(AVFormatContext *s, int64_t timestamp, int flags)
{
	int res = FALSE;
	if(s == NULL || s->pb == NULL )
		return AVERROR(32);
	...
	if(timestamp>= 0) //有效的时间戳
	{
		...
		type = get_byte(s->pb);
		printf("%s	line %d ,type %d, pos 0x%x\n",__FUNCTION__,__LINE__,type,pos);
			...
			if (!url_feof(s->pb) )
			{
				...
				if(pts < timestamp) //前进搜索
				{
					for(;;)
					{
						...
						pts = get_be24(s->pb);
						pts |= (uint32_t)get_byte(s->pb) << 24;
						//printf("%s size %d pts %lld\n",__FUNCTION__,size,pts);
						if (url_feof(s->pb) )
							break;//AVERROR_EOF;
						stream_id = get_be24(s->pb);/* stream id, always 0 */
						...
						if (type == FLV_TAG_TYPE_VIDEO) 
						{
							...
							size--;
							//printf("flags 0x%x\n",flags);
							if(flags & FLV_FRAME_KEY)
							{
								seekpos = url_ftell(s->pb) - 16;
								res = TRUE;
							}
						}
						next= size + url_ftell(s->pb);
				...
				else		//后退搜索
				{
					...
					for(;;)
					{
						...
						type = get_byte(s->pb);
						packetsixe = get_be24(s->pb);
						//printf("%s	line %d ,type %d, size %d,packetsixe %d \n",__FUNCTION__,__LINE__,type,size,packetsixe);
						if(oldsize ==  packetsixe+ 11)
						{
							pts = get_be24(s->pb);
							pts |= (uint32_t)get_byte(s->pb) << 24;
							//printf("%s size %d pts %lld\n",__FUNCTION__,size,pts);
							...
							{
								flags = get_byte(s->pb);
								if((flags & FLV_FRAME_KEY) && pts <= timestamp)
								{
									seekpos = url_ftell(s->pb) - 16 ;
									res = TRUE;
									break;
...
int flv_read_pre_packet(AVFormatContext * s, AVPacket * pkt)
{
    int ret;
	int64_t nextpre,next;
	unsigned int size;
	static int64_t  oldvalidPos = 0;

    nextpre = url_ftell(s->pb);    
    url_fskip(s->pb, -4);
    
    size = get_be32(s->pb);
	next = nextpre - size - 4;	//next previous tag
	if (next <= 0)
	{
	   url_fseek(s->pb, oldvalidPos, SEEK_SET);
	   return AVERROR(32);
	}
	else
	{
	    oldvalidPos = nextpre;
	    url_fseek(s->pb, nextpre, SEEK_SET);
	   // my_printf(LEVEL_DEBUG,"read %x\n",*(s->pb->buf_ptr));
	    ret = flv_read_packet(s,pkt);
	    url_fseek(s->pb, next, SEEK_SET);
	    return ret;
	}
