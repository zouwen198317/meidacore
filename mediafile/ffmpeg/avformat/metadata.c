...
int av_metadata_set2(AVMetadata **pm, const char *key, const char *value, int flags)
{
    ...
    if(tag){
        ...
        *tag= m->elems[--m->count];
        ...
        AVMetadataTag *tmp= av_realloc_t(m->elems, (m->count+1) * sizeof(*m->elems));
        if(tmp){
            m->elems= tmp;
        }else
    ...
    if(value){
        ...
        m->elems[m->count].key  = av_strdup_t(key  );
        if(flags & AV_METADATA_DONT_STRDUP_VAL){
            m->elems[m->count].value= (char *)value;
        }else
        m->elems[m->count].value= av_strdup_t(value);
        m->count++;
    }
    if(!m->count) {
        av_free_t(m->elems);
...
void av_metadata_free(AVMetadata **pm)
{
    ...
    if(m){
		//printf("m->count %d\n",m->count);
        while(m->count--){
            av_free_t(m->elems[m->count].key);
            av_free_t(m->elems[m->count].value);
        }
        av_free_t(m->elems);
    }
    av_freep_t(pm);
