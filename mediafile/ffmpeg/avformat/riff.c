...
int ff_put_wav_header(ByteIOContext *pb, AVCodecContext *enc)
{
    ...
    if(bps != enc->bits_per_coded_sample && enc->bits_per_coded_sample){
        ...
        printf( "requested bits_per_coded_sample (%d) and actually stored (%d) differ\n", enc->bits_per_coded_sample, bps);
    }
...
void ff_get_wav_header(ByteIOContext *pb, AVCodecContext *codec, int size)
{
    ...
    id = get_le16(pb);
    codec->codec_type = CODEC_TYPE_AUDIO;//AVMEDIA_TYPE_AUDIO;
    codec->codec_tag = id;
//printf("%s line %d codec_tag %x \n",__FUNCTION__,__LINE__,id);
    ...
    if (size >= 18) {  /* We're obviously dealing with WAVEFORMATEX */
        ...
        if (cbSize > 0) {
            codec->extradata = av_mallocz_t(codec->extradata_size + FF_INPUT_BUFFER_PADDING_SIZE);
...
//printf("%s line %d codec_tag %x codec_id %x\n",__FUNCTION__,__LINE__,id,codec->codec_id);
}
...
//		printf("%s line %d time_base.num %d,time_base.den %d !!!!\n",__FUNCTION__,__LINE__,stream->time_base.num,stream->time_base.den);
             
        *au_scale= stream->time_base.num;
        *au_rate = stream->time_base.den;
//		printf("%s line %d gcd %d *au_scale %d,*au_rate %d\n",__FUNCTION__,__LINE__,gcd,*au_scale,*au_rate);
    }else{
	...
	a = *au_scale;
	b = *au_rate;
//	printf("%s line %d gcd %d *au_scale %lld,*au_rate %lld\n",__FUNCTION__,__LINE__,gcd,a,b);
    gcd= av_gcd(a, b);
