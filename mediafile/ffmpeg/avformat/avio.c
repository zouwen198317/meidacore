...
URLProtocol *first_protocol = &file_protocol;
					   ...
					   const char *filename, int flags)
{
	...
	uc = (URLContext *)malloc(sizeof(URLContext) + strlen(filename) + 1);
	if (!uc) {
		err = AVERROR(ENOMEM);
		goto fail;
	}
...
int url_write(URLContext *h, unsigned char *buf, int size)
{
	...
	{
		printf("%s line %d: url_write() error\n", __FUNCTION__, __LINE__);
		return AVERROR(EIO);
	}
	/* avoid sending too big packets */
	if (h->max_packet_size && size > h->max_packet_size)
	{
		printf("%s line %d: url_write() sending too big packets\n", __FUNCTION__, __LINE__);
		return AVERROR(EIO);
	}
	ret = h->prot->url_write(h, buf, size);
	if(ret < 0)
		printf("%s line %d: url_write() return ret %d\n", __FUNCTION__, __LINE__,ret);
	return ret;
}
