...
static void build_filter(FELEM *filter, double factor, int tap_count, int phase_count, int scale, int type){
    ...
    for(ph=0;ph<phase_count;ph++) {
        ...
        for(i=0;i<tap_count;i++) {
            ...
            default:
                w = 2.0*x / (factor*tap_count*M_PI);
                y *= bessel(type*sqrt(FFMAX_T(1-w*w, 0)));
                break;
            }
...
#else
            filter[ph * tap_count + i] = av_clip_t(lrintf(tab[i] * scale / norm), FELEM_MIN, FELEM_MAX);
#endif
        }
    }
#if 0
...
int av_resample(AVResampleContext *c, short *dst, short *src, int *consumed, int src_size, int dst_size, int update_ctx){
  ...
  if(compensation_distance == 0 && c->filter_length == 1 && c->phase_shift==0){
        ...
        dst_size= FFMIN_T(dst_size, (src_size-1-index) * (int64_t)c->src_incr / c->dst_incr);
    ...
    for(dst_index=0; dst_index < dst_size; dst_index++){
        ...
        if(sample_index < 0){
            for(i=0; i<c->filter_length; i++)
                val += src[FFABS_T(sample_index + i) % src_size] * filter[i];
    ...
    *consumed= FFMAX_T(index, 0) >> c->phase_shift;
