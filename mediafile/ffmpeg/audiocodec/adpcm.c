...
AVFrame *avcodec_alloc_frame(void){
    AVFrame *pic= av_malloc_t(sizeof(AVFrame));
...
int adpcm_encode_init(AVCodecContext *avctx)
{
    ...
    case CODEC_ID_ADPCM_IMA_WAV:
        ...
        avctx->bits_per_coded_sample = 4;   //add 
    ...
    case CODEC_ID_ADPCM_MS:
        ...
        //printf("CODEC_ID_ADPCM_MS frame size %d\n", avctx->frame_size);//2036
        extradata = avctx->extradata = av_malloc_t(avctx->extradata_size);
        if (!extradata)
            return (- ENOMEM);
    ...
    case CODEC_ID_ADPCM_SWF:
        if (avctx->sample_rate != 11025 &&
            ...
            printf("Sample rate must be 11025, 22050 or 44100\n");
            return -1;
        }
...
{
    //av_freep_t(&avctx->coded_frame);
    if(avctx->coded_frame)av_free_t(avctx->coded_frame);
    if(avctx->extradata) av_free_t(avctx->extradata);
	avctx->extradata = NULL;
    return 0;
}
...
static inline unsigned char adpcm_ima_compress_sample(ADPCMChannelStatus *c, short sample)
{
    ...
    int nibble = FFMIN_T(7, abs(delta)*4/step_table[c->step_index]) + (delta<0)*8;
    c->prev_sample += ((step_table[c->step_index] * yamaha_difflookup[nibble]) / 8);
    c->prev_sample = av_clip_int16_t(c->prev_sample);
    c->step_index = av_clip_t(c->step_index + index_table[nibble], 0, 88);
    return nibble;
}
...
static inline unsigned char adpcm_ms_compress_sample(ADPCMChannelStatus *c, short sample)
{
    ...
    nibble= av_clip_t(nibble, -8, 7)&0x0F;

    predictor += (signed)((nibble & 0x08)?(nibble - 0x10):(nibble)) * c->idelta;

    c->sample2 = c->sample1;
    c->sample1 = av_clip_int16_t(predictor);
...
static inline unsigned char adpcm_yamaha_compress_sample(ADPCMChannelStatus *c, short sample)
{
    ...
    nibble = FFMIN_T(7, abs(delta)*4/c->step) + (delta<0)*8;

    c->predictor += ((c->step * yamaha_difflookup[nibble]) / 8);
    c->predictor = av_clip_int16_t(c->predictor);
    c->step = (c->step * yamaha_indexscale[nibble]) >> 8;
    c->step = av_clip_t(c->step, 127, 24567);
    ...
    for(i=0; i<n; i++) {
        ...
        for(j=0; j<frontier && nodes[j]; j++) {
            ...
            if(version == CODEC_ID_ADPCM_MS) {
                ...
                const int nmin = av_clip_t(div-range, -8, 6);
                const int nmax = av_clip_t(div+range, -7, 7);
...
#define STORE_NODE(NAME, STEP_INDEX)\
                    int d;\
                    uint32_t ssd;\
                    dec_sample = av_clip_int16_t(dec_sample);\
                    d = sample - dec_sample;\
                    ssd = nodes[j]->ssd + d*d;\
                    ...
                    next_##NAME:;
                    STORE_NODE(ms, FFMAX_T(16, (AdaptationTable[nibble] * step) >> 8));
                }
            } else if((version == CODEC_ID_ADPCM_IMA_WAV)|| (version == CODEC_ID_ADPCM_IMA_QT)|| (version == CODEC_ID_ADPCM_SWF)) {
#define LOOP_NODES(NAME, STEP_TABLE, STEP_INDEX)\
                const int predictor = nodes[j]->sample1;\
                const int div = (sample - predictor) * 4 / STEP_TABLE;\
                int nmin = av_clip_t(div-range, -7, 6);\
                int nmax = av_clip_t(div+range, -6, 7);\
                ...
                LOOP_NODES(ima, step_table[step], av_clip_t(step + index_table[nibble], 0, 88));
            } else { //CODEC_ID_ADPCM_YAMAHA
                LOOP_NODES(yamaha, step, av_clip_t((step * yamaha_indexscale[nibble]) >> 8, 127, 24567));
#undef LOOP_NODES
#undef STORE_NODE
            }
        }
...
//printf("%s line %d\n",__FUNCTION__,__LINE__);
        ...
        n = (avctx->frame_size - 1) / 8; //add
            c->status[0].prev_sample = (signed short)samples[0]; /* XXX */
/*            c->status[0].step_index = 0; *//* XXX: not sure how to init the state machine */
            bytestream_put_le16(&dst, c->status[0].prev_sample);
            if(c->status[0].step_index < 0)
                c->status[0].step_index = 0;
            else if(c->status[0].step_index > 88)
...
//		printf("%s line %d\n",__FUNCTION__,__LINE__);
        ...
        for(i=0; i<avctx->channels; i++){
            c->status[i].step_index = av_clip_t(c->status[i].step_index, 0, 63); // clip step so it fits 6 bits
...
static inline short adpcm_ima_expand_nibble(ADPCMChannelStatus *c, char nibble, int shift)
{
    ...
    c->predictor = av_clip_int16_t(predictor);
...
static inline short adpcm_ct_expand_nibble(ADPCMChannelStatus *c, char nibble)
{
    ...
    c->step = av_clip_t(new_step, 511, 32767);
...
static inline short adpcm_sbpro_expand_nibble(ADPCMChannelStatus *c, char nibble, int size, int shift)
{
    ...
    /* clamp result */
    c->predictor = av_clip_t(c->predictor + (sign ? -diff : diff), -16384,16256);
                            ...
                            AVPacket *avpkt)
{
    ...
    if (!buf_size)
        return 0;
/*
printf("%s line %d,CODEC_ID_ADPCM_SWF %x,avctx->codec->id %x  In data:\n",__FUNCTION__,__LINE__,CODEC_ID_ADPCM_SWF,avctx->codec->id);
printf("%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",*(buf),*(buf+1),*(buf+2),*(buf+3),*(buf+4),
	*(avpkt->data+5),*(avpkt->data+6),*(avpkt->data+7),*(avpkt->data+8),*(avpkt->data+9));
*/
    //should protect all 4bit ADPCM variants
    //8 is needed for CODEC_ID_ADPCM_IMA_WAV with 2 channels
    //
//printf("%s line %d, *data_size/4 =%d, buf_size + 8 %d\n",__FUNCTION__,__LINE__,*data_size/4,buf_size + 8);
    ...
    *data_size= 0;
    ...
    case CODEC_ID_ADPCM_IMA_QT:
        ...
        for (channel = 0; channel < avctx->channels; channel++) {
            ...
            cs->predictor = av_clip_int16_t(cs->predictor);
...
//        samples_per_block= (block_align-4*chanels)*8 / (bits_per_sample * chanels) + 1;
        ...
        for(i=0; i<avctx->channels; i++){
            ...
            if (*src++) printf( "unused byte should be null but is %d!!\n", src[-1]); /* unused */
        }
    ...
    case CODEC_ID_ADPCM_MS:
        ...
        if (n < 0)
            return -1;
		//printf("%s line %d\n",__FUNCTION__,__LINE__);
        block_predictor[0] = av_clip_t(*src++, 0, 6);
        block_predictor[1] = 0;
        if (st)
            block_predictor[1] = av_clip_t(*src++, 0, 6);
    ...
    case CODEC_ID_ADPCM_EA:
        ...
        for (count1 = 0; count1 < samples_in_chunk/28;count1++) {
            ...
            for (count2 = 0; count2 < 28; count2++) {
                ...
                current_left_sample = av_clip_int16_t(next_left_sample);
                previous_right_sample = current_right_sample;
                current_right_sample = av_clip_int16_t(next_right_sample);
        ...
        for (count1 = 0; count1 < (buf_size - avctx->channels) / avctx->channels; count1++) {
            for(i = 4; i >= 0; i-=4) { /* Pairwise samples LL RR (st) or LL LL (mono) */
                for(channel = 0; channel < avctx->channels; channel++) {
                    ...
                    c->status[channel].sample1 = av_clip_int16_t(sample);
            ...
            for (count1=0; count1<samples_in_chunk; count1++) {
                if (*srcC == 0xEE) {  /* only seen in R2 and R3 */
                    ...
                    for (count2=0; count2<28; count2++) {
                        ...
                        next_sample = av_clip_int16_t(next_sample >> 8);
            ...
            for (m=2; m<32; m+=2) {
                ...
                for (n=0; n<4; n++, src++, s+=32*avctx->channels) {
                    for (s2=s, i=0; i<8; i+=4, s2+=avctx->channels) {
                        ...
                        s2[0] = av_clip_int16_t((level + pred + 0x80) >> 8);
                    }
                }
            }
        }
...
//printf("CODEC_ID_ADPCM_SWF avctx->channels %d,nb_bits %d\n",avctx->channels,nb_bits);
        while (get_bits_count(&gb) <= size - 22*avctx->channels) {
            ...
            for (count = 0; get_bits_count(&gb) <= size - nb_bits*avctx->channels && count < 4095; count++) {
                ...
                for (i = 0; i < avctx->channels; i++) {
                    ...
                    c->status[i].step_index = av_clip_t(c->status[i].step_index, 0, 88);
                    c->status[i].predictor = av_clip_int16_t(c->status[i].predictor);

                    *samples++ = c->status[i].predictor;
                    if (samples >= samples_end) {
                        printf( "allocated output buffer is too small\n");
                        return -1;
                    }
                }
            }
        ...
        for (ch = 0; ch <= st; ch++) {
            ...
            for (i = 0; i < samplecnt / 14; i++) {
                ...
                for (n = 0; n < 14; n++) {
                    ...
                    *samples = av_clip_int16_t(sampledat);
                    prev[ch][1] = prev[ch][0];
                    prev[ch][0] = *samples++;
